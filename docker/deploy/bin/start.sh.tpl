...
if [[ "${deploy_fisco_bcos}x" == "yesx" ]]; then
    ...
    bash "${deploy_root}/util/wait_tcp.sh" -p 20200 -t 60 \
        ...
        -f "FISCO-BCOS nodes start failed!! Check error in log: [ ${deploy_root}/fiscobcos/nodes/127.0.0.1/node0/log/ ]"
fi
...
if [[ "${deploy_webase_front}x" == "yesx" ]]; then
    ...
    bash "${deploy_root}/util/wait_tcp.sh" -p ${webase_front_port} -t 60 \
        ...
        -f "WeBASE-Front start failed!! Check error in log: [ ${deploy_root}/webase/log/WeBASE-Front-error.log ]"
fi

...
abstract contract FiscoOracleClient {
  ...
  event Requested(bytes32 indexed id,address oracleAddress,uint256 requestCount,string datasource);
  ...
  returns (bytes32 requestId) {
    ...
    requestCount++;
    reqc[msg.sender]++;
    emit Requested(requestId,_oracle,requestCount,datasource);
    return requestId;
  }

...
contract VRFK1CoreWithBlockHash is VRFUtil, Ownable {
   ...
   * @dev block.
   */
  function fulfillRandomnessRequest(uint256[2] memory _publicKey, bytes memory _proof, uint256 preSeed, uint blockNumber,bytes32 blockHash) public {

    (bytes32 currentKeyHash, Callback memory callback, bytes32 requestId,
    uint256 randomness) = getRandomnessFromProof(_publicKey, _proof, preSeed, blockNumber,blockHash);
  ...
  function getRandomnessFromProof( uint256[2] memory _publicKey, bytes memory _proof, uint256 preSeed, uint blockNumber,bytes32 blockHash)
  internal view returns (bytes32 currentKeyHash, Callback memory callback,
	...
	//bugtrace,有的版本blockhash实现有问题，注释，从客户端传过来
